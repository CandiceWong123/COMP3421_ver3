...
function fetchLocalWeather(iconObj, conditionObj, reportObj) {
    getWeather('local').then(data => {
        ...
        let weatherImage = getWeatherImage(weather.weather_en);

        if (weatherImage !== "unknown") {
            if (iconObj) iconObj.src = weatherImage; 
            if (conditionObj) {conditionObj.innerText = document.documentElement.lang == "en"? weather.weather_en : weather.weather_tc;}
        ...
        listItem.innerHTML = `
            <div class="forecast_summary">
                ...
                <span class="f_temp_range">
                    <img class="f_icon" src=${getWeatherImage(determineWeather(day.forecastWeather).weather_en)} alt="Weather Icon">
...
function determineWeather(forecast){
    ...
    if (isSunny){
        if(isRainny){
            return {"weather_en":"Drizzle", "weather_tc":"細雨"};
        }else if(isCloudy){
            return {"weather_en":"Partly cloudy", "weather_tc":"局部多雲"};
        }
        return {"weather_en":"Sunny", "weather_tc":"陽光普照"};
    }
    else if (isRainstorm){
        return {"weather_en":"Rainstorm", "weather_tc":"暴雨"};
    }
    else if (isRainny){
        return {"weather_en":"Shower rain", "weather_tc":"陣雨"};
    }
    else if (isCloudy){
        return {"weather_en":"Cloudy", "weather_tc":"多雲"};
    }
    console.log("Unknown weather condition");    
    return {"weather_en":"unknown", "weather_tc":"未知"}; 
}
...
async function loadTranslations(lang) {
    ...
    fetchLocalWeather(null,document.getElementById("c_condition"),document.getElementById('c_report'));
